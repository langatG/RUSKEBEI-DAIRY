...
Begin VB.Form frmreceipts 
   ...
   Begin VB.CommandButton cmdTranz 
      Caption         =   "Transporters Enquiry"
      BeginProperty Font 
         Name            =   "News701 BT"
         Size            =   9
         Charset         =   0
...
Else
lblSNames = ""
MsgBox "the supplier doesnot exist please select who buy", vbInformation
Exit Sub
tt:
End If
gg:
...
txtamount = 0
End If
''**********checking if B.Price is > than the S.Price **********
If CCur(txtbuyingprice) >= CCur(txtsellingprice) Then
...
Private Sub txttranscode_Validate(Cancel As Boolean)
Set rs = New ADODB.Recordset
Dim DTPfrom As Date
sql = "d_sp_TransEnquiry  '" & txttranscode & "'"
Set rs = oSaccoMaster.GetRecordset(sql)
...
If Not IsNull(rs.Fields(0)) Then lbltransportername = rs.Fields(0)
Else
MsgBox "Transporter doesnot exist", vbInformation
Exit Sub
End If
DTPfrom = DateSerial(year(txtransdate), month(txtransdate), 1)
DTPto = DateSerial(year(txtransdate), month(txtransdate) + 1, 1 - 1)
