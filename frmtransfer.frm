...
sql = "SELECT     UserLoginID, UserGroup, SUPERUSER,status From UserAccounts where UserLoginID='" & User & "'"
    ...
    If Not rs.EOF Then
    If rs!status <> 1 Then
    MsgBox "You are not allowed to transfer kilos", vbInformation
...
End If
If txtreason = "" Then
MsgBox "Please Write reason for transfer", vbCritical
Exit Sub
End If
If txtsno1 = "" Then
...
sql = "d_sp_MilkIntake " & txtSNo & ",'" & DTPTransferDate & "'," & txtkgs * -1 & "," & Price & "," & Price * CCur(txtkgs * -1) & ",'" & Time & "','" & User & "','Transfer To " & txtsno1 & "','" & txtreason & "'"
oSaccoMaster.ExecuteThis (sql)
'//receipient
Set cn = New ADODB.Connection
sql = "d_sp_MilkIntake " & txtsno1 & ",'" & DTPTransferDate & "'," & txtkgs & "," & Price & "," & Price * CCur(txtkgs) & ",'" & Time & "','" & User & "','Transfer from " & txtSNo & "','" & txtreason & "'"
oSaccoMaster.ExecuteThis (sql)
