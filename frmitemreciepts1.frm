...
Begin VB.Form frmitemreciepts 
   ...
   ClientTop       =   435
   ClientWidth     =   10665
   Icon            =   "frmitemreciepts1.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   8835
   ...
   Begin VB.CommandButton cmdstckre 
   ...
   Begin VB.ComboBox ports 
      Height          =   315
      ItemData        =   "frmitemreciepts1.frx":000C
      Left            =   9480
      List            =   "frmitemreciepts1.frx":0022
      TabIndex        =   30
      Text            =   "COM1"
      Top             =   8400
      Width           =   855
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(0)   =   "frmitemreciepts1.frx":003A
      ...
      TabPicture(1)   =   "frmitemreciepts1.frx":0056
...
''Set rs = oSaccoMaster.GetRecordset("SELECT     *  FROM         dbo.d_LPO where vendor = '" & cboVendor & "'")
Set rs = oSaccoMaster.GetRecordset("SELECT     dbo.d_LPO.PNo, dbo.d_LPO.TransDate, dbo.d_Requisition.IName,d_Requisition.pricing, dbo.d_Requisition.Qnty FROM         dbo.d_LPO INNER JOIN   dbo.d_Requisition ON dbo.d_LPO.RefNo = dbo.d_Requisition.RNo WHERE     (dbo.d_Requisition.Status = 'Ordered')  and vendor = '" & cboVendor & "' ")
While Not rs.EOF
Set li = Lvwitems.ListItems.Add(, , rs.Fields("PNo"))
                li.SubItems(1) = rs.Fields("TransDate") & ""
                li.SubItems(2) = rs.Fields("IName") & ""
                li.SubItems(3) = rs.Fields("pricing") & ""
                li.SubItems(4) = rs.Fields("Qnty") & ""
                li.SubItems(5) = "0" & ""
                li.SubItems(6) = rs.Fields("Qnty") & ""
...
If txtQnty = "" Then
    MsgBox "enter quantity delivered"
    Exit Sub
End If
...
sql = ""
sql = "set dateformat DMY select * from  d_Invoice where RNo='" & txtRef & "' "
Set rsst = New ADODB.Recordset
Set rsst = oSaccoMaster.GetRecordset(sql)
If Not rsst.EOF Then
sql = ""
sql = "set dateformat dmy update d_Invoice set InvId='" & txtDelNo & "' ,InvDate='" & dtprecDate & "' where RNo='" & txtRef & "' "
...
''''''If Not rsst.EOF Then
''''''mysql = ""
''''''mysql = "set dateformat DMY INSERT INTO ag_stockbalance"
''''''mysql = mysql & " (p_code, productname, openningstock, changeinstock, stockbalance, transdate,companyid)"
''''''mysql = mysql & " VALUES     ('" & pcode & "', '" & LvwselectedItems.ListItems(j).SubItems(2) & "', '" & rs.Fields("o_bal") & "', '" & qty & "', '" & rs.Fields("o_bal") + qty & "', '" & Format(Get_Server_Date, "dd/mm/yyyy") & "',1)"
''''''oSaccoMaster.Execute (mysql)
''''''Else
''''''mysql = ""
''''''mysql = "set dateformat DMY INSERT INTO ag_stockbalance"
''''''mysql = mysql & " (p_code, productname, openningstock, changeinstock, stockbalance, transdate,companyid)"
''''''mysql = mysql & " VALUES     ('" & pcode & "', '" & LvwselectedItems.ListItems(j).SubItems(2) & "', '0', '" & qty & "', '" & qty & "', '" & Format(Get_Server_Date, "dd/mm/yyyy") & "',1)"
''''''oSaccoMaster.Execute (mysql)
''''''
''''''End If
...
'
ReceiptNo = Trim$(li)
sql = "Update d_Requisition set status='Receipt' ,Qnty='" & qty & "' where RNo='" & txtRef & "'"
...
Set rs = oSaccoMaster.GetRecordset("SELECT  CompanyName  FROM ag_Supplier1 order by companyname asc")
While Not rs.EOF
